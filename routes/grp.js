                        ...
                        connection.query('SELECT g_idx,g_pwd,g_date from groups where g_name=?',
                            [grpjoinData.gname], function (err2, results) {
                                if (err2) {
                                    ...
                                    callback(null, {
                                        gidx: results[0].g_idx,
                                        gpwd: results[0].g_pwd,
                                        gdate: results[0].g_date
                                    });//gidx, gpwd 넘겨줌
                                }
                                connection.release();
                            });//query
                            ...
                            connection.query('INSERT INTO account_has_group(account_a_idx,group_g_idx,g_score,g_game,g_joindate) VALUES(?,?,?,?,now())',
                                [aidx, arg1.gidx, 0, 0], function (err2, results) {
                                    if (err2) {
                                        ...
                                        callback(null, {
                                            result: "SUCCESS",
                                            resultmsg: "success group join",
                                            gidx: arg1.gidx,
                                            gname: grpjoinData.gname
                                        });
                                    }
                                    else {
                                            ...
                                            connection.query('SELECT account_a_idx FROM account_has_group where group_g_idx=?',
                                                [grpdelData.gidx], function (err2, result) {
                                                    if (err2) {
                                                        ...
                                                        res.json({
                                                            result: "FAIL",
                                                            resultmsg: "NO MEMBER ON DELETE GRP"
                                                        });
                                                        connection.release();
                                                        return;
                                                    }
            ...
            function (callback) {
                db.pool.getConnection(function (err, connection) {
                    if (err) {
                        ...
                        connection.query('SELECT g_idx,g_name,g_photo,g_master,DATE_FORMAT(g_date,"%Y-%m-%d") g_date FROM groups WHERE g_name LIKE ?',
                            [grpsearchData.gname + "%"],
                            function (err2, result) {
                                if (err2) {
        ...
        function (callback) {
            db.pool.getConnection(function (err, connection) {
                if (err) {
                    ...
                    connection.query('SELECT a.a_idx,a.name,a.prophoto,ag.league_avg l_avg FROM account AS a' +
                        ' LEFT OUTER JOIN account_has_group AS ag ON a.a_idx = ag.account_a_idx WHERE ag.group_g_idx' +
                        ' IS NOT null AND ag.group_g_idx=? ORDER BY l_avg DESC',
                        [leagueData.gidx],
        ...
        function (arg, callback) {
            ...
            db.pool.getConnection(function (err, connection) {
                if (err) {
                    ...
                    connection.query('SELECT AVG(league_avg) allavg FROM account AS a' +
                        ' LEFT OUTER JOIN account_has_group AS ag ON a.a_idx = ag.account_a_idx' +
                        ' WHERE ag.group_g_idx IS NOT null AND ag.group_g_idx=?',
                        [leagueData.gidx],
                        function (err2, results) {
                            if (err2) {
                                ...
                                callback(null, {
                                    arr: arr,
                                    allavg: (results[0].allavg == null || results[0].allavg == null) ? 0 : parseFloat(results[0].allavg).toFixed(1)
                                });
                            }
                            else {
                                console.log('no data');
        ...
        if (err) {
            ...
            db.pool.getConnection(function (err, connection) {
                if (err) {
                    ...
                    connection.query('SELECT a.prophoto prophoto,ag.league_avg l_avg FROM account AS a ' +
                        'LEFT OUTER JOIN account_has_group AS ag ON a.a_idx = ag.account_a_idx ' +
                        'WHERE ag.group_g_idx IS NOT null AND ag.group_g_idx=? AND ag.account_a_idx=? ',
